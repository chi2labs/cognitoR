...
The CognitoServer Module will communicate via OAUTH with Amazon Cognito. if application received a code will get a token via oauth (code can be used only once). 

When application already have the token (via url or received with using the code) will check authorization with Amazon Cognito via OAUTH request using the token, if authorization is valid will load your Shiny application, otherwise user will be redirected again to login page.
