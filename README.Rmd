...
[![Build Status](https://travis-ci.org/chi2labs/cognitoR.svg?branch=master)](https://travis-ci.org/chi2labs/cognitoR)
<!-- badges: end -->
...
Inspired on an initial contribution done by <a href="https://adisarid.github.io/post/2019-08-10-cognito-shiny-authentication" target="_blank">Adi Sarid</a>.
...
When an user access to your Shiny application with CognitoR, the user is redirected to your configured url in Amazon Cognito, there if user is not logged will see the login page.

If user is already logged or authentication is succesful in the login page, the user is redirected again to your Shiny App with a code/token (depending of your configuration) in the url. 

If application load receiving a code will get a token via oauth (code can be used only once). When application already have the token (via url or received using the code) will check authorization with Amazon Cognito via OAUTH request using the token, if authorization is valid will load your Shiny application, otherwise user will be redirected again to login page.
...
Go to App Integration -> Domain Name, there you must to set the url for login form, you can use a Amazon subdomain or use your own domain.
...
This package require that you have a configuration file ("config.yml") in your application folder with the following structure:
...
In the example mentioned above includes the suse of the Logout module (```logout_ui()``` and ```logout_server()```) which provide a "logout" button interacting with the reactive "isLogged" returned from Cognito Module to show the button and with the logout callback when button is pressed.
